...
# -*- coding: utf-8 -*-
"""
evaluate_full_run_with_limiter_relative.py - Evaluation script for DTLN + Relative Transient Limiter.
Applies transient_limiter.py in use_relative_thresh=True mode (relative_margin_db=10.0, threshold_db=-3.0)
to all 120 DTLN validation outputs.
Saves data_full/eval_results_full_run_with_limiter_relative.csv and prints 3-way & 4-way comparative metrics.
"""
...
def main():
    print("=== DTLN + Relative Transient Limiter Evaluation (Held-Out Validation Set) ===")
    ...
    chained_output_dir = r"d:\SIH 2026\DTLN\data_full\enhanced_full_run_with_limiter_relative"
    csv_out_path = r"d:\SIH 2026\DTLN\data_full\eval_results_full_run_with_limiter_relative.csv"
    ...
    print("Applying relative transient limiter to DTLN outputs & computing metrics...")
    ...
    for idx, dtln_path in enumerate(dtln_files):
        ...
        # Apply relative transient limiter
        lim_sig, gain_curve = apply_transient_limiter(
            ...
            use_relative_thresh=limiter_params['use_relative_thresh'],
            relative_margin_db=limiter_params['relative_margin_db']
        )
    ...
    print(f"\nFull per-file relative evaluation saved to: {csv_out_path}")
    
    # Trigger Statistics
    snr_levels = sorted(list(set(r['input_snr_cond'] for r in results)))
    print("\n" + "="*85)
    print("Relative Limiter Triggering Statistics by SNR Level")
    print("="*85)
    print(f"{'Input SNR':<10} | {'Triggered Files / Total':<25} | {'Trigger Rate':<15} | {'Avg Min Gain':<15}")
    print("-"*85)
    for snr_val in snr_levels:
        sub = [r for r in results if r['input_snr_cond'] == snr_val]
        trig_cnt = sum(1 for r in sub if r['triggered'])
        rate = (trig_cnt / len(sub)) * 100.0
        avg_g = np.mean([r['min_limiter_gain'] for r in sub])
        print(f"{snr_val:<10.1f}dB | {trig_cnt:>2}/{len(sub):<2}                     | {rate:<14.1f}% | {avg_g:<15.4f}")
    print("="*85)
    
    # 3-Way Comparative Summary Table by Input SNR Level (Relative Limiter)
    print("\n" + "="*115)
    print("3-WAY COMPARISON BY SNR LEVEL: Noisy vs. DTLN Only vs. DTLN + Relative Limiter (+10dB Margin)")
    print("="*115)
    print(f"{'Input SNR':<10} | {'SI-SNR (dB) [Noisy / DTLN / Relative]':<40} | {'STOI [Noisy / DTLN / Relative]':<32} | {'PESQ [Noisy / DTLN / Relative]':<30}")
    print("-"*115)
    ...
    # 4-Way Comparison across all Limiter Modes
    ...
    print("\n" + "="*125)
    print("4-WAY COMPREHENSIVE COMPARISON: DTLN Only vs. Static -6dB vs. Static -3dB vs. Relative (+10dB)")
    print("="*125)
    print(f"{'Input SNR':<10} | {'SI-SNR: DTLN / -6dB / -3dB / Rel':<36} | {'STOI: DTLN / -6dB / -3dB / Rel':<34} | {'PESQ: DTLN / -6dB / -3dB / Rel':<32}")
    print("-"*125)
    
    for snr_val in snr_levels:
        sub_rel = [r for r in results if r['input_snr_cond'] == snr_val]
        ...
        sd = np.mean([r['si_snr_dtln'] for r in sub_rel])
        ...
        sr = np.mean([r['si_snr_chained'] for r in sub_rel])
        
        td = np.mean([r['stoi_dtln'] for r in sub_rel])
        ...
        tr = np.mean([r['stoi_chained'] for r in sub_rel])
        
        pd = np.mean([r['pesq_dtln'] for r in sub_rel])
        ...
        pr = np.mean([r['pesq_chained'] for r in sub_rel])
        
        print(f"{snr_val:<10.1f}dB | {sd:>5.2f}/{s6:>5.2f}/{s3:>5.2f}/{sr:>5.2f}               | {td:.3f}/{t6:.3f}/{t3:.3f}/{tr:.3f}             | {pd:.3f}/{p6:.3f}/{p3:.3f}/{pr:.3f}")
    ...
    o_sr = np.mean([r['si_snr_chained'] for r in results])
    ...
    o_tr = np.mean([r['stoi_chained'] for r in results])
    ...
    o_pr = np.mean([r['pesq_chained'] for r in results])
    
    print(f"{'OVERALL':<10} | {o_sd:>5.2f}/{o_s6:>5.2f}/{o_s3:>5.2f}/{o_sr:>5.2f}               | {o_td:.3f}/{t6:.3f}/{o_t3:.3f}/{o_tr:.3f}             | {o_pd:.3f}/{o_p6:.3f}/{o_p3:.3f}/{o_pr:.3f}")
    print("="*125)
